...
import { createGridByName, getGridById, getGridsByUser, updateCellValueById } from "../../service/gridService";
import type { APIError } from '../../types/error';
import type { Grid, GridCell } from '../../types/grid';

export const fetchGridsByUser = createAsyncThunk<
  Grid[],
  string,
  { rejectValue: APIError }
>("grids/fetchByUser", async (username, { rejectWithValue }) => {
  try {
...
export const fetchGridById = createAsyncThunk<
  ...
  string,
  { rejectValue: APIError }
>("grids/fetchById", async (id, { rejectWithValue }) => {
  try {
...
export const createGrid = createAsyncThunk<
  ...
  string,
  { rejectValue: APIError }
>("grids/create", async (name, { rejectWithValue }) => {
  try {
