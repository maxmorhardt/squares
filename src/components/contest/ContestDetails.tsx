import { BugReport, Info, Lock, SportsScore } from '@mui/icons-material';
...
export default function ContestDetails({ isOwner = false }: ContestDetailsProps) {
  ...
  const showLockButton = isActive && allSquaresFilled;
  const showScoreInputs = isLocked || isInGame;
  ...
  const handleLock = () => {
    // TODO: Dispatch action to lock contest
          ...
          <>
            ...
            {showLockButton && (
              <>
                ...
                >
                  Lock the contest to start the game. Numbers will be randomly assigned.
                </Typography>
              </>
            )}
            ...
            {isActive && !allSquaresFilled && (
              <>
                ...
                >
                  Waiting for all squares to be filled before you can lock the contest.
                </Typography>
                {isDev && (
                  <Button
                    variant="outlined"
            ...
            {/* Locked/In-Game State - Score Inputs */}
            {showScoreInputs && (
              <>
                <Divider sx={{ borderColor: 'rgba(255,255,255,0.1)' }} />
